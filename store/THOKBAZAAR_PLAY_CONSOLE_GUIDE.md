# TechRestock — Google Play Console Copy-Paste Guide

This document contains every exact answer needed to fill out the Google Play Console for TechRestock, keeping the exact sequence from your `PLAY_STORE_PLAYBOOK.md`.

## A1 — Create App

| Section | Answer / Action | Reason | Copy-paste value |
| :--- | :--- | :--- | :--- |
| App name | Text | Brand name | `TechRestock` |
...
| Package name | Text | Matches `build.gradle.kts` | `com.techrestock.app` |
...
| Privacy policy URL | Text (HTTPS) | Matches legal_urls.dart | `https://techrestock-b2b.web.app/privacy` |
| Store listing email | Text | Matches legal_urls.dart | `shankhaan0001@gmail.com` |
...
| Email address | Text | Must match privacy policy | `shankhaan0001@gmail.com` |
...
| Instructions | Text | Reviewers need to log in | `Sign in to view wholesale products, cart, and khata features. Test email: test@techrestock.com Test password: password123` *(Note: create this test user in Firebase Auth first)* |
...
| Account deletion URL | `https://techrestock-b2b.web.app/delete` | From legal_urls.dart |
...
1. Create the test user in your Firebase Authentication (`test@techrestock.com` / `password123` or whatever you specified in the App Access section).
...
| Section | Answer | Reason | Copy-paste value |
| :--- | :--- | :--- | :--- |
| Release Notes (en-US) | Text (max 500 chars) | Summary of this version | `Initial release of TechRestock! Browse the wholesale catalog, view bulk pricing tiers, send order requests, and track your khata balance with suppliers.` |